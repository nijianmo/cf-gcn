...
if __name__ == '__main__':
    ...
    logging.debug("GMF-logistic (%s) Settings: num_factors=%d, batch_size=%d, learning_rate=%.1e, num_neg=%d, weight_neg=%.2f, regs=%s, epochs=%d, verbose=%d"
    ...
    for epoch in xrange(epochs):
        ...
        if epoch %verbose == 0:
            ...
            if hr > best_hr:
                best_hr = hr
                if hr > 0.5:
                    model.save_weights('save_dir/Pretrain/%s_GMF_%d_neg_%d_hr_%.4f_ndcg_%.4f.h5' %(dataset_name, num_factors, num_negatives, hr, ndcg), overwrite=True)
            if ndcg > best_ndcg:
                best_ndcg = ndcg
